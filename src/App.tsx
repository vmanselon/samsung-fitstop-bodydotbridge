...
export default function App() {
  ...
  const handleCode = useCallback(async (code: string) => {
    ...
    try {
      ...
      setSaveNotice(`${payload.nickname}님의 결과를 저장했습니다.${RUNTIME.useMockSave ? " (DEBUG/MOCK)" : ""}`);
      leaveScanner();
  ...
  return (
    <main className="kiosk-page result-page">
      ...
      <button className="primary-button" type="button" onClick={() => setPage("scanner")}>결과 저장</button>
      {saveNotice && <div className="save-notice" role="status">{saveNotice}</div>}
      <div className="checker checker--bottom" aria-hidden="true" />
      {RUNTIME.useMockData && <aside className="debug-tools"><span>DEBUG · {result.measurementId}</span><button type="button" onClick={simulateNewResult}>새 Bodydot 결과</button></aside>}
    </main>
  );
}
